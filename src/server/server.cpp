...
int tcp_connect(int &list_socket, struct sockaddr_in &serv_addr);
...
if (!tcp_connect(list_socket, serv_addr)){
  std::cout << "Error tcp_connect:" << strerror(errno) << '\n';
  return -1;
}
...
int client_task(int client_socket) {
  ...
  int read_byte = 0;
    ...
    std::cout << "//execute_command(data_pointers, client_socket);" << std::endl;
