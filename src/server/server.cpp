...
int main(int argc, char **argv)
{
  ...
  if((bind(listening_socket, (pointer_socket *)&server_address, sizeof(server_address))) == -1)
  {
    printf("Error bind():%s\n", strerror(errno));
    return -1;
  }
  ...
  while(1)
  {
    ...
    if((client_socket = accept(listening_socket, (pointer_socket *)&client_address, &length)) == -1)
    {
  ...
  inet_ntop(AF_INET, &(client_address.sin_addr), addr_clients, INET_ADDRSTRLEN);
